...
class CompleteGraph:
    ...
    def reachable_nodes(self, starting_node: Node) -> List[Node]:
    ...
    def is_connected(self) -> bool:
        ...
        reachable_nodes: List[Node, ...] = self.reachable_nodes(start)
        #print(len(reachable_nodes),len(self.graph))
        return len(reachable_nodes) == len(self.graph)

    def draw_circular_graph(self, node_color='pink', node_size=1000):
        """Draw a circular graph of the current nodes/edges."""
